...
- Order module contents by category: module docstring and `__future__` imports; standard-library, third-party, and local imports; public constants; private constants and module state; public classes and functions; then private classes and helpers.
- Default to private names. Use a leading underscore for implementation details, internal constants, and helpers unless external callers need a stable API.
- Keep the supported public surface explicit with `__all__` when a module exposes an intentional API. Do not re-export names from a package's `__init__.py` without a concrete need.
- Use `UPPER_CASE` only for supported cross-module constants. Use `_UPPER_CASE` for internal defaults.
- Use double underscores only to avoid subclass name collisions, not as the default privacy mechanism.
- A public function may call a private helper defined later in the module. Names required during module initialization, such as decorators, base classes, and default argument values, must be defined first.
